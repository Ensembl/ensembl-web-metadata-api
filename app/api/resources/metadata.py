...
from api.models.genome import Release
...
async def get_releases(
    ...
    try:
        releases_stream = db.get_release(
...
def test_get_releases():
    response = client.get("/releases?release_name=1")
    assert response.status_code == 200
    assert response.json() == {"releaseLabel": "1", "releaseType":"partial","isCurrent":False}
